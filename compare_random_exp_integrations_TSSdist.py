...
'''
    USAGE: python compare_random_exp_integrations_TSSdist.py <TSS_ref_file> <exp_integrations_file> <database>
'''
...
def main():
    ...
    for i in range(2):
        ...
        out_sim = open("distances_sim_to_TSS_per_bin.tsv", 'w')
        ...
        hist, bin_edges = np.histogram(selected_distances, density = False, bins = 40, range=[-200000, 200000])    
    ...
    out_exp = open("distances_exp_to_TSS_per_bin.tsv", 'w')
    ...
    hist_exp, bin_edges_exp = np.histogram(selected_distances_exp, density = False, bins = 40, range=[-200000, 200000])
    ...
    printer('nearestTSS_stats_1000simulations.xlsx', df)


    fig, ax = plt.subplots() 
    kwargs = dict(histtype='bar', color='green', density=False, bins=40,  range=[-200000, 200000],
        edgecolor='black', linewidth=1)
    ax.hist(selected_distances_exp, **kwargs)

    kwargs = dict(histtype='step', color='white', density=False, bins=40,  range=[-200000, 200000],
        edgecolor='black', linewidth=1, alpha=0.9)
    ### ax.hist(list_of_averages, **kwargs)
    ax.hist(range(-195000, 200000, 10000), weights = list_of_averages, **kwargs)
    ax.hist(range(-195000, 200000, 10000), weights = list_of_lower_CI, **kwargs)
    ax.hist(range(-195000, 200000, 10000), weights = list_of_upper_CI, **kwargs)

    new_ticks_labels = ['-200', '-150', '-100', '-50', '0', '50', '100', '150', '200']
    x_locator   = FixedLocator([-200000, -150000, -100000, -50000, 0, 50000, 100000, 150000, 200000])
    ...
    fig.savefig("retrovirus_integrations_nearestTSS_1000-simulations.png")
