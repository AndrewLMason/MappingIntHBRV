...
def main():
    ...
    if db == 'all':
        number_int = 5006
        output_file_name_prefix = f'all_integrations_dist_nearestTSS_{num_iter}-simulations_60K'
    elif db == 'invitro':
        number_int = 3386
        output_file_name_prefix = f'invitro_integrations_dist_nearestTSS_{num_iter}-simulations_60K'
    elif db == 'invivo':
        number_int = 1620
        output_file_name_prefix = f'invivo_integrations_dist_nearestTSS_{num_iter}-simulations_60K'
    ...
    for i in range(num_iter):
        ...
        hist, bin_edges = np.histogram(selected_distances, density = False, bins = 30, range=[-60000, 60000])    
    ...
    hist_exp, bin_edges_exp = np.histogram(selected_distances_exp, density = False, bins = 30, range=[-60000, 60000])
    ...
    fig, ax = plt.subplots() 
    kwargs = dict(histtype='bar', color='green', density=False, bins=30,  range=[-60000, 60000],
        edgecolor='black', linewidth=1)
    ax.hist(selected_distances_exp, **kwargs)

    kwargs = dict(histtype='step', color='white', density=False, bins=30,  range=[-60000, 60000],
        edgecolor='black', linewidth=1, alpha=0.9)
    ### ax.hist(list_of_averages, **kwargs)
    ax.hist(range(-57000, 60000, 4000), weights = list_of_averages, **kwargs)
    ax.hist(range(-57000, 60000, 4000), weights = list_of_lower_CI, **kwargs)
    ax.hist(range(-57000, 60000, 4000), weights = list_of_upper_CI, **kwargs)

    new_ticks_labels = ['-60', '-40', '-20', '0', '20', '40', '60']
    x_locator   = FixedLocator([-60000, -40000, -20000,  0, 20000, 40000, 60000])
...
def sort_by_bin(integrations_list):
    dict_of_lists = {}
    for i in range (-60000, 60000, 4000):
        minimum = i
        maximum = i + 4000
